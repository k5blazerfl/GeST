...
# --- generate a fresh fstab for an install target ---------------------------
#
# When GeST has just mounted a set of new filesystems under a target root (an
# install), it writes that system's own /etc/fstab from scratch, keyed by UUID
# (stable across device renumbering). The per-filesystem defaults below follow
# the Gentoo Handbook's example fstab.

FSTAB_HEADER = (
    "# /etc/fstab: generated by GeST\n#\n"
    "# <fs>\t<mountpoint>\t<type>\t<opts>\t<dump/pass>\n"
)


def _entry_defaults(mountpoint: str, fstype: str) -> tuple[str, int]:
    """The (options, passno) defaults for a generated entry."""
    if fstype == "swap":
        return "sw", 0
    if fstype == "vfat":           # the EFI system partition
        return "umask=0077", 2
    if mountpoint == "/":
        return "noatime", 1
    return "defaults", 2
...
def gen_fstab_entry(uuid: str, mountpoint: str, fstype: str) -> FstabEntry:
    """Build a UUID-keyed :class:`FstabEntry` for a generated fstab.

    ``mountpoint`` is the mount path on the installed system (``/``, ``/efi``, …)
    or ``"none"`` for swap. Options and pass number are chosen from ``fstype`` and
    ``mountpoint`` (see :func:`_entry_defaults`).
    """
    options, passno = _entry_defaults(mountpoint, fstype)
    return FstabEntry(f"UUID={uuid}", mountpoint, fstype, options, 0, passno)


def render_fstab(entries: list[FstabEntry]) -> str:
    """Render a whole fstab (header + one line per entry) for a target system."""
    body = "\n".join(render_entry(e) for e in entries)
    return FSTAB_HEADER + (body + "\n" if body else "")


def valid_generated_fstab(text: str) -> bool:
    """True if ``text`` parses to at least one entry and every entry is valid.

    The gate both privileged fstab writers (direct and backend) run before
    committing a generated install-target fstab to disk.
    """
    entries = parse_fstab(text)
