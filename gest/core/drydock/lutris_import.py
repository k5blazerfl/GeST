...
* **native**  — a 1:1 Drydock step (or barrel state);
...
def convert(script: Mapping) -> ImportResult:
    ...
    barrel = R.RecipeBarrel(runner=RUNNER_WINE, arch=arch,
                            dxvk=_as_bool(wine_cfg.get("dxvk")),
                            vkd3d=_as_bool(wine_cfg.get("vkd3d")))

    # system.env → barrel env; everything else under system.* is HeDE's job.
    system = _as_mapping(script.get("system"))
    barrel.env = {str(k): str(v) for k, v in _as_mapping(system.get("env")).items()}
    for key in system:
        if key != "env":
    ...
    for directive in _as_list_of(script.get("installer")):
        _convert_directive(directive, steps, barrel, warnings, rejected)
    ...
    recipe = R.Recipe(
        ...
        categories=["Game"], barrel=barrel, files=files, steps=steps,
        programs=programs, prereqs="auto",
    )
    return ImportResult(recipe=recipe, warnings=warnings, rejected=rejected)


def _convert_directive(directive, steps, barrel, warnings, rejected) -> None:
    ...
    if name in _NATIVE_DIRECTIVES:
        ...
        _convert_task(params, steps, barrel, warnings, rejected)
...
def _convert_task(params, steps, barrel, warnings, rejected) -> None:
