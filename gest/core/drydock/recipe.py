...
# interpreter (phase 6, ``_plan_step``) dispatches all of them — ``execute``,
# ``regedit``, ``regdelete``, and ``eject_disc`` now plan to real commands
# (``wine``/``umu-run``/``reg``/``eject``); only ``manual`` needs a human.
