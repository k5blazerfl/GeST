"""``make.conf`` shell-assignment grammar: parse and format-preserving render.

``NAME="value"`` lines with optional quoting, backslash line-continuations, and
``${VAR}`` parameter references (which are preserved, never expanded). This is
the proven parser from ``core/makeconf/reader.py``, lifted here as a pure codec
so both the reader (preview) and the backend (write) share one implementation.
"""
...
def assignments(text: str) -> list[tuple[str, str, int, int]]:
    """Raw ``(name, value, start_line, end_line)`` for every assignment, in order."""
    lines = text.split("\n")
...
def variables(text: str) -> list[Var]:
    ...
    order: list[str] = []
    values: dict[str, str] = {}
    for name, value, _s, _e in assignments(text):
...
def render(text: str, name: str, value: str) -> str:
    """Return the text with ``name`` set to ``value``.

    Replaces the effective (last) assignment in place, or appends it if absent.
    Every comment, blank line, and unrelated assignment is preserved verbatim.
    """
    lines = text.split("\n")
    assigns = [a for a in assignments(text) if a[0] == name]
