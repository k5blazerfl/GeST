...
- **`gest-0.48.1`** — the latest released version (urwid; depends only on
  `::gentoo` packages), from the `v0.48.1` tag tarball (`~amd64`). Recommended —
  green SUSE-nod palette; Software Management distinguishes source- vs
  binary-installed packages (ⓑ glyph + Origin detail) and can install the binary
  version (--getbinpkg), with binhost sources merged into its Binary packages
  menu.
