...
- **`gest-0.48.0`** — the latest released version (urwid; depends only on
  `::gentoo` packages), from the `v0.48.0` tag tarball (`~amd64`). Recommended —
  Software Management distinguishes source- vs binary-installed packages (ⓑ
  glyph + Origin detail), can install the binary version (--getbinpkg), with
