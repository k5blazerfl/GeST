...
from gest.core.drydock import bottles, launch, prereq
...
def _proton_bottle(**over) -> Bottle:
...
# ---- model -------------------------------------------------------------
def test_bottle_round_trip():
    b = _wine_bottle()
    b.programs[0].graphics = GraphicsProfile(gamescope=True, fsr=True, fps_cap=60)
    back = Bottle.from_dict(b.to_dict())
    assert back == b


def test_bottle_is_valid_and_program_lookup():
    b = _wine_bottle()
    assert b.is_valid()
    assert not Bottle(id="x", name="x", runner="nope").is_valid()
    assert not Bottle(id="x", name="x", runner=RUNNER_WINE, arch="win128").is_valid()
...
def test_store_round_trip_and_default_prefix(tmp_path):
    b = Bottle(id="office", name="Office", runner=RUNNER_WINE)  # no prefix
    path = bottles.save_bottle(b, base=str(tmp_path))
    assert path.exists()
    # save filled in the default prefix under the bottle dir
    assert b.prefix == str(bottles.default_prefix("office", str(tmp_path)))
    assert bottles.list_bottles(base=str(tmp_path)) == ["office"]
    assert bottles.load_bottle("office", base=str(tmp_path)) == b
    assert bottles.delete_bottle("office", base=str(tmp_path)) is True
    assert bottles.list_bottles(base=str(tmp_path)) == []


def test_slug_and_unsafe_id():
    assert bottles.slug("My Office!") == "my-office"
    assert bottles.slug("") == "bottle"
    for bad in ("../x", "A/B", "Up.Case"):
        try:
            bottles.bottle_dir(bad, base="/tmp")
        except ValueError:
            continue
...
def test_prereqs_wine_multilib_and_toggles():
    b = _wine_bottle(arch="win32", dxvk=True, vkd3d=True)
    ...
    b64 = _wine_bottle(arch="win64", dxvk=False, vkd3d=False)
