...
def test_wine_script_converts_to_recipe():
    ...
    assert rec.barrel.runner == RUNNER_WINE
    assert rec.barrel.arch == ARCH_WIN32
    assert rec.barrel.dxvk is True
...
def test_winetricks_verbs_collected_into_barrel():
    rec = lutris_import.convert(_wine_script()).recipe
    assert "dotnet48" in rec.barrel.verbs


def test_default_arch_is_win64():
    rec = lutris_import.convert(_wine_script(wine={})).recipe
    assert rec.barrel.arch == ARCH_WIN64
...
def test_system_env_mapped_other_system_flagged():
    ...
    assert result.recipe.barrel.env == {"DXVK_HUD": "fps"}
...
def test_recipe_dict_shape_matches_adr_sketch():
    ...
    assert set(d) == {"recipe", "app", "barrel", "files", "steps", "programs", "prereqs"}
