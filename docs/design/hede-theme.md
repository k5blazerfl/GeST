...
> `frame.png` decoration above is **not achievable server-side today** — it would
> need client-side decorations or a compositor change (a separate architectural
> call). The shipped approximation is a **top-lit vertical gradient** titlebar
> (accent → accent ×0.82), generated by `helm-theme` into the Helm themerc — the
> frost/bevel of the contract within labwc's real capability. The window chrome
> follows the active world: `helm-theme --from-world` (run from the labwc
> autostart) seeds the themerc from the world's accent, mirroring the shell's
> live `helm::effectiveAccent` precedence.
