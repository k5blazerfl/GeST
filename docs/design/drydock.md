...
**A Windows program installed into a Drydock bottle should feel like a native
...
already applied.** The bottle setup is where the work lives; everything after is
...
| **umu-launcher** (`umu-run`) | Runs Proton *outside* Steam: the SLR container + Proton + **protonfixes** (per-game fixes) generically | **The foundation for Proton bottles.** We shell to `umu-run`, we don't reimplement the runtime |
...
| **Bottles** | Prefixes as first-class **managed bottles** with presets/dependencies; **exports shortcuts** to the desktop | The bottle model + dependency (winetricks) verbs + desktop export — but export via **Customs**, deeper than a bare `.desktop` |
...
> Running Windows software is not an emulator project. It is **(a)** a bottle
...
- **Proton via umu** (`games-util/umu-launcher`, **GURU overlay** — not in-tree):
  ...
  bundles DXVK + VKD3D-Proton** — so Proton bottles need *no* separate DXVK setup.
...
- **Wayland:** `winewayland.drv` is improving but **opt-in, not the default**;
  ...
  the Wine-Wayland driver only where a bottle opts in.

## 4. The bottle model

A **bottle** = a managed prefix + config, under `$XDG_DATA_HOME/hede/drydock/<id>/`:
...
- **verbs**: winetricks verbs installed (wine bottles).
- **dxvk/vkd3d**: state (wine bottles only — Proton has them built in).
...
`drydock-run <bottle> <program>` assembles a **pure** (env, argv) pair, then execs:
...
1. **env** — `WINEPREFIX`, `WINEARCH`; `WINEESYNC/WINEFSYNC` per profile; DXVK/
   ...
   using gamescope**; plus the bottle's env overrides.
...
1. **Synthesize** (controlled): on registering a program, build a Customs
   `.desktop` (Exec → `drydock-run <bottle> <program>`), extract its icon with
...
2. **Harvest** (convenience): scan a bottle for the `.desktop` files wine's
...
A **prerequisites checker**: given a bottle's config, report exactly what's needed
and missing —
- Proton bottle → `games-util/umu-launcher` (**offer to enable the GURU overlay**
  via GeST's repos module) + `media-libs/…` Vulkan.
- Wine bottle → `app-emulation/wine-vanilla[abi_x86_32]` (+ **multilib /
...
- **Pure/testable now:** bottle model + config store; the launch env+argv builder
